  ...
  context 'windows' do
    ...
    it 'fetches java' do
      expect(chef_run).to run_ruby_block(
        'fetch http://download.oracle.com/otn-pub/java/jdk/8u65-b17/jdk-8u65-windows-x64.exe')
    end

    it 'validates java' do
      expect(chef_run).to run_ruby_block('validate C:/chef/cache/jdk-8u65-windows-x64.exe')
    end

    it 'installs java' do
      expect(chef_run).to run_execute('install jdk-8u65-windows-x64.exe to C:\Program Files\Java\jdk1.8.0_65 '\
                                      'with JRE C:\Program Files\Java\jre1.8.0_65')
    end
    ...
    it 'creates link to JDK bin' do
      expect(chef_run).to create_link('C:\java\jdk\bin').with(
        to: 'C:\Program Files\Java\jdk1.8.0_65\bin'
      )
    end
  end
end
