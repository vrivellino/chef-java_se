  ...
  context 'mac_os_x' do
    ...
    it 'fetches java' do
      expect(chef_run).to run_ruby_block(
        'fetch http://download.oracle.com/otn-pub/java/jdk/8u66-b17/jdk-8u66-macosx-x64.dmg')
    end

    it 'validates java' do
      expect(chef_run).to run_ruby_block('validate /var/chef/cache/jdk-8u66-macosx-x64.dmg')
    end

    it 'attaches volume' do
      expect(chef_run).to run_execute("hdiutil attach '/var/chef/cache/jdk-8u66-macosx-x64.dmg' -quiet")
    end

    it 'install pkg' do
      expect(chef_run).to run_execute("sudo installer -pkg '/Volumes/JDK 8 Update 66/JDK 8 Update 66.pkg' -target /")
    end

    it 'detaches volume' do
      expect(chef_run).to run_execute("hdiutil detach '/Volumes/JDK 8 Update 66' " \
        "|| hdiutil detach '/Volumes/JDK 8 Update 66' -force")
    end
    ...
    it 'adds BundledApp capability' do
      expect(chef_run).to run_execute('/usr/bin/sudo /usr/libexec/PlistBuddy -c ' \
        ...
        '/Library/Java/JavaVirtualMachines/jdk1.8.0_66.jdk/Contents/Info.plist')
    end
    ...
    it 'adds current jdk' do
      expect(chef_run).to run_execute('/usr/bin/sudo /bin/ln -nsf ' \
        '/Library/Java/JavaVirtualMachines/jdk1.8.0_66.jdk/Contents ' \
        '/System/Library/Frameworks/JavaVM.framework/Versions/CurrentJDK')
    end

    it 'creates java home' do
      expect(chef_run).to run_execute('/usr/bin/sudo /bin/ln -nsf ' \
        '/Library/Java/JavaVirtualMachines/jdk1.8.0_66.jdk/Contents/Home /Library/Java/Home')
    end

    it 'creates lib dir' do
      expect(chef_run).to run_execute('/usr/bin/sudo /bin/mkdir -p ' \
        '/Library/Java/JavaVirtualMachines/jdk1.8.0_66.jdk/Contents/Home/bundle/Libraries')
    end

    it 'creates java home' do
      expect(chef_run).to run_execute('/usr/bin/sudo /bin/ln -nsf ' \
        '/Library/Java/JavaVirtualMachines/jdk1.8.0_66.jdk/Contents/Home/jre/lib/server/libjvm.dylib ' \
        '/Library/Java/JavaVirtualMachines/jdk1.8.0_66.jdk/Contents/Home/bundle/Libraries/libserver.dylib')
    end
  end
end
